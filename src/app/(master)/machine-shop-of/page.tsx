...
const page = () => {
    ...
    const { data, addData, updateData, getItemById, deleteItem } = useFetchData(service?.API_URL?.machineShopOf?.listing)
        ...
        },
        {
            ...
            selector: (row: any) => {
                return <><ActionButton edit history deleteAction
                    onClick={() => handleEdit(row?.machineShopOfId)}
                    handleConfirmDelete={() => handleOpenDelete(row?.machineShopOfId)}
                /> </>
            },
            sortable: false,
            width: '10rem',
    ...
    const onSubmit: SubmitHandler<FormData> = async (data: FormData) => {
        const body = {
            ...data,
            machineShopOfId: machineShopId ? machineShopId : 0,
            isDelete: false,
        }
        if (machineShopId) {
            await updateData(body, service?.API_URL?.machineShopOf.update, reset)
        } else {
            await addData(body, service?.API_URL?.machineShopOf.add, reset)
        }
    }
    ...
    const handleEdit = async (id: any) => {
        ...
        try {
            setMachineShopId(id)
            const editMachineRes: any = await getItemById(id, service.API_URL.machineShopOf.getbyid);
            if (editMachineRes) {
                dispatch(openModal());
    ...
    return (
        <>
            ...
            />
            <Filter data={FilterArray.machineshopofFilter} />
            <ReusableTable
                columns={columns}
                data={data}
            />
            ...
            <Modal
                ...
                body={
                    <div className="grid grid-cols-3 gap-4">
                        <div>
                            <label htmlFor="machineShopOfName" className="block text-gray-700">
                                Name <span className="text-red-500">*</span>
                            </label>
                            <input
                                {...register('machineShopOfName', {
                                    required: 'This field is required',
                                })}
                                id="machineShopOfName"
                                type="text"
                                className="block w-full px-3 py-2 border rounded mt-2"
                            />
                            {errors?.machineShopOfName?.message && (
