...
const page = () => {
    ...
    const { data, addData, updateData, deleteItem, getItemById }: any = useFetchData(service?.API_URL?.documentTypeMaster.listing)
    ...
    const columns = [
        ...
        {
            name: 'Document Type',
            selector: (row: any) => row.documentTypeName,
            sortable: true,
            width: '75rem'
        },
        {
        ...
        },
        {
            ...
            selector: (row:any) => {
                ...
                onClick={() => handleEdit(row?.documentTypeId)}
                handleConfirmDelete={() => handleOpenDelete(row?.documentTypeId)}
                /> </>
            },
            sortable: false,
            width: '10rem',
    ...
    const onSubmit: SubmitHandler<FormData> = async (data: FormData) => {
        const body = {
            ...data,
            documentTypeId: documentTypeId ? documentTypeId : 0,
            isDelete: false,
        }
        if (documentTypeId) {
            await updateData(body, service?.API_URL?.documentTypeMaster.update, reset)
        } else {
            await addData(body, service?.API_URL?.documentTypeMaster.add, reset)
        }
    }
    ...
    const handleEdit = async (id: any) => {
        ...
        try {
            ...
            const editMachineRes: any = await getItemById(id, service.API_URL.documentTypeMaster.getbyid);
            if (editMachineRes) {
                dispatch(openModal());
    ...
    const handleConfirmDelete = async () => {
        if (documentTypeId) {
            await deleteItem(documentTypeId)
        }
    }
    return (
        <>
            ...
            <Modal
                ...
                body={
                    <div className="grid grid-cols-3 gap-4">
                        <div>
                            <label htmlFor="documentTypeName" className="block text-gray-700">
                            ...
                            />
                            {errors?.documentTypeName?.message && (
