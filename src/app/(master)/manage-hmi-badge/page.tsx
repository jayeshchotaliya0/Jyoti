...
const page = () => {
  ...
  const { data, addData, updateData, deleteItem, getItemById }: any = useFetchData(service?.API_URL?.hmiBadge.listing)
  ...
  const columns = [
    ...
    {
      name: 'HMI Badge Name',
      selector: (row: any) => row.hmiBadgeName,
      sortable: true,
      width: '75rem',
    },
    {
    ...
    },
    {
      ...
      selector: (row: any) => {
        return <><ActionButton edit history deleteAction
          onClick={() => handleEdit(row?.hmiBadgeId)}
          handleConfirmDelete={() => handleOpenDelete(row?.hmiBadgeId)}
        /> </>
      },
      sortable: false,
      width: '10rem',
  ...
  const onSubmit: SubmitHandler<FormData> = async (data: FormData) => {
    const body = {
      ...data,
      hmiBadgeId: hmiId ? hmiId : 0,
      isDelete: false,
    }
    if (hmiId) {
      await updateData(body, service?.API_URL?.hmiBadge.update, reset)
    } else {
      await addData(body, service?.API_URL?.hmiBadge.add, reset)
    }
  }
  ...
  const handleEdit = async (id: any) => {
    ...
    try {
      setHmiId(id)
      const editMachineRes: any = await getItemById(id, service.API_URL.hmiBadge.getbyid);
      if (editMachineRes) {
        dispatch(openModal());
  ...
  return (
    <>
      ...
      />
      <Filter data={FilterArray.hmiBudgeFilter} />
      <ReusableTable columns={columns} data={data} />
      <Modal
        mdlTitle="Add HMI Badge"
        btnSubmit="Save"
        ...
        body={
          ...
          <div className="grid grid-cols-3 gap-4">
            <div>
              <label htmlFor="hmiBadgeName" className="block text-gray-700">
                HMI Badge Name <span className="text-red-500">*</span>
              </label>
              <input
                {...register('hmiBadgeName', {
              ...
              />
              {errors?.hmiBadgeName?.message && (
                <span className="text-red-500">{errors?.hmiBadgeName?.message}</span>
              )}
            </div>
            <div>
